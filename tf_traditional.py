...
#%%
x_train, y_train, x_test, y_test = load_data(path, **trad1, label=label)
if sys.argv[1] == '1':
    x_train, y_train, x_test, y_test = load_data(path, **trad1, label=label)
if sys.argv[1] == '2':
    x_train, y_train, x_test, y_test = load_data(path, **trad2, label=label)
...
loss, metric = model.evaluate(x_test, y_test)
print(metric)
with open(f'results/tf_traditional_{sys.argv[1]}-ieee.txt', 'a') as f:
    f.write(f'{metric[0]}\n')