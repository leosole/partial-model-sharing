...
def train(model, opts, trainloader, epochs, device, criterion, metric_fn=ps.f1_score):
    ...
    for i in range(epochs):
        ...
        for x, y in trainloader:
            ...
            actuals.extend(y.long().data.numpy())
            predictions.extend(labels.data.numpy())
        metric = metric_fn(actuals, predictions) 
        history['metric'].append(metric)
        print(f'\rTRAIN [{i/epochs*100:.1f}%] F1: {metric:.4f} \t Acc: {num_right/len(trainloader.dataset)}\t Loss: {loss:.4f}', end='')
    return history

def test(model, testloader, criterion, device, metric_fn=ps.accuracy, print_test=True):
    loss = 0.0
    num_right = 0
    model.eval()
    with torch.no_grad():
...
print()
_, f1_score = test(model, testloader, criterion, DEVICE, print_test=True)
print('Client DONE!')
# if len(sys.argv) > 1:
