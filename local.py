...
def train(model, opts, trainloader, epochs, device, criterion, metric_fn=ps.f1_score):
    ...
    for i in range(epochs):
        ...
        for x, y in trainloader:
            ...
            actuals.extend(y.long())
            predictions.extend(labels.long())
        # metric = num_right / len(trainloader.dataset)
        metric = metric_fn(actuals, predictions) 
...
if sys.argv[1] == '1':
    trainloader, testloader, num_examples, n_columns = ps.load_data(config.path, **config.client1)
    ind_columns = [col for col in config.client1['columns'] if col not in shared_columns]

# if sys.argv[1] == '2':
trainloader, testloader, num_examples, n_columns = ps.load_data(config.path, **config.client2)
ind_columns = [col for col in config.client2['columns'] if col not in shared_columns]

model = ps.Net([n_columns, *config.local_layers], config.dropout)
...
print('Client DONE!')
if len(sys.argv) > 1:
    with open(f'results/local_client-{sys.argv[1]}.txt', 'a') as f:
        f.write(f'{metric}\n')
else:
    with open(f'results/local_client-.txt', 'a') as f:
        f.write(f'{metric}\n')
# %%
